...
public class MainService extends Service {
    ...
    private void checkAppsUpdates() {
        if(!DataStorage.isAssistantInstall()) {
            requestInstallAssistant();
        }else{
            if(!checkAssistantUpdate()){
                checkMainAppUpdate();
            }
        }
    }

    private void checkMainAppUpdate() {
        if (!DataStorage.getAppAvailableVersion().isEmpty()) {
            if (DataStorage.needUpdate(BuildConfig.VERSION_NAME, DataStorage.getAppAvailableVersion())) {
                if (DataStorage.getAppUpdateFile().exists()) {
                    requestUpdateApp();
                } else {
                    if(!DialogActivity.isShow){
                        RemoteServerHelper.getINSTANCE().completeTask(RemoteServerHelper.TaskType.getAppNewVer);
                    }
                }
            }
        }
    ...
    private boolean checkAssistantUpdate() {
        if (!DataStorage.getAppAssistantAvailableVersion().isEmpty() && !DataStorage.getAssistantCurVersion().isEmpty()) {
            if (DataStorage.needUpdate(DataStorage.getAssistantCurVersion(), DataStorage.getAppAssistantAvailableVersion())) {
                if (DataStorage.getAssistantUpdateFile().exists()) {
                    requestUpdateAssistant();
                } else {
                    if(!DialogActivity.isShow){
                        RemoteServerHelper.getINSTANCE().completeTask(RemoteServerHelper.TaskType.getAssistantNewVer);
                        return true;
                    }
                }
            }
    ...
    private void requestUpdateAssistant() {
        if (PermissionHelper.hasWriteExtStoragePermission()) {
    ...
    private void requestInstallAssistant() {
        if (PermissionHelper.hasWriteExtStoragePermission()) {
            if (DataStorage.lastShowAssistanrMsgTime.get() + DataStorage.SHOW_ASSISTANR_MSG_INT < System.currentTimeMillis()) {
                DataStorage.lastShowAssistanrMsgTime.set(System.currentTimeMillis());
